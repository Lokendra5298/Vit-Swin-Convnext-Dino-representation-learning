...
@torch.no_grad()
def topk_accuracy(logits: torch.Tensor, targets: torch.Tensor, topk: Tuple[int, ...] = (1, 5)) -> List[torch.Tensor]:
...
def update_confusion_matrix(confusion: torch.Tensor, preds: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
    """Update confusion matrix where rows=true labels and columns=predictions."""
...
def compute_metrics_from_confusion(confusion: torch.Tensor, class_names: Sequence[str]) -> Dict[str, object]:
    ...
    eps = 1e-12
    precision = correct_per_class / np.maximum(pred_per_class, eps)
    recall = correct_per_class / np.maximum(true_per_class, eps)
    f1 = 2 * precision * recall / np.maximum(precision + recall, eps)
    support = true_per_class

    total = cm.sum()
    accuracy = correct_per_class.sum() / max(total, eps)
